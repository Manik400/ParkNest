...
public sealed class WalletsController : ControllerBase
{
    ...
    public WalletsController(IWalletService wallets, ILedgerService ledger)
    {
        _wallets = wallets;
        _ledger = ledger;
    }
    ...
    /// <summary>Real money in. In production this is called by the payment aggregator's webhook,
    /// never directly by the client.</summary>
    ...
    [HttpPost("{userId:guid}/cash-out")]
    public async Task<ActionResult<CashOutResponse>> CashOut(
        Guid userId,
        [FromBody] CashOutRequest request,
        CancellationToken cancellationToken)
    {
        var payout = await _wallets.RequestCashOutAsync(userId, request.Amount, request.IdempotencyKey, cancellationToken);
        return Ok(new CashOutResponse(payout.Id, payout.Amount, payout.Status.ToString()));
    }

    /// <summary>Reconciliation view: balances recomputed from the entry stream.</summary>
